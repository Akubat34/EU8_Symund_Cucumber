...
public class Tasks_StepDefinitions  {
        ...
        public void user_types_to_the_placeholder_which_includes_add_a_current_task_to_to_do_message_and_user_hits_the_enter(String string) {
                tasksPage.taskPlaceHolder.sendKeys("First Task", Keys.ENTER);
        }
        ...
        @Then("Click the checkbox left side of the task name")
        public void click_the_checkbox_left_side_of_the_task_name() {
        }
        @Then("Verify that user can add task into completed tasks")
        public void verify_that_user_can_add_task_into_completed_tasks() {
                /*System.out.println(tasksPage.checked.getAttribute("class"));
                Assert.assertTrue(tasksPage.checked.getAttribute("class").contains("Done"));
